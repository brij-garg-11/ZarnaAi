...
def _render_quality_tab() -> str:
    ...
    for idx, r in enumerate(reports):
        ...
        # ── Detail body (hidden until expanded) ──────────────────────────────
        # Stats strip
...
@admin_bp.route("/admin/quality/<int:report_id>/review", methods=["POST"])
def quality_mark_reviewed(report_id: int):
    """Mark a quality digest report as reviewed."""
    if not check_admin_auth():
        return require_admin_auth_response()
    conn = get_db_connection()
    if not conn:
...
@admin_bp.route("/admin/quality/run", methods=["GET"])
def quality_run_digest():
    """Trigger the quality digest script inline (streams output)."""
    if not check_admin_auth():
        return require_admin_auth_response()
