"""
Newsletter Responses tab — track fan replies to a newsletter CTA.

Each newsletter issue includes a CTA: "text the bot your answer to <question>".
This tab has two views, mirroring the Podcast Q&A tab:

  1. Campaign list  — one campaign per newsletter issue; click a card to open it
  2. Campaign detail — two sections:
       • "To Review"  — incoming responses the AI isolated; check off the real ones
       • "Confirmed"  — the ones you checked off; mark them "featured" once used
                        in a future newsletter

The campaign stores the newsletter date AND the CTA question, and both are fed
to the detection scan so the AI can isolate replies to that specific prompt.

Routes registered via register_newsletter_routes(bp) from app/admin/__init__.py.
"""
...
#   confirmed — a real response you picked  → shows in "Confirmed"
#   featured  — confirmed AND already used in a newsletter → "Confirmed" (dimmed)
#   skip      — dismissed as not a real response → hidden
_STATUSES = ("new", "confirmed", "featured", "skip")
_REVIEW_STATUSES = ("new",)
_CONFIRMED_STATUSES = ("confirmed", "featured")
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute("""
                ...
                LEFT JOIN newsletter_submissions s ON s.campaign_id = c.id
                GROUP BY c.id
                ORDER BY c.created_at DESC
            """)
            return [dict(r) for r in cur.fetchall()]
    except Exception:
        logger.exception("[ADMIN] Failed to load newsletter campaigns")
        return []
    finally:
        conn.close()
...
def _get_campaign(campaign_id: int) -> dict | None:
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(
                "SELECT id, label, newsletter_date, question, created_at "
                "FROM newsletter_campaigns WHERE id = %s",
                (campaign_id,),
            )
            row = cur.fetchone()
            return dict(row) if row else None
    except Exception:
        logger.exception("[ADMIN] Failed to load newsletter campaign")
        return None
    finally:
        conn.close()
...
def _get_submissions(campaign_id: int, statuses: tuple) -> list:
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute("""
                SELECT s.id, s.phone_number, s.message_id, s.response,
        ...
        logger.exception("[ADMIN] Failed to load newsletter submissions")
        return []
    finally:
        conn.close()


def _create_campaign(label: str, newsletter_date, question: str) -> bool:
    conn = _get_db()
    if not conn:
        return False
    try:
        with conn:
            with conn.cursor() as cur:
                cur.execute(
                    "INSERT INTO newsletter_campaigns (label, newsletter_date, question) "
                    "VALUES (%s, %s, %s)",
                    (label.strip(), newsletter_date or None, question.strip()),
                )
        return True
    except Exception:
        logger.exception("[ADMIN] Failed to create newsletter campaign")
        return False
    finally:
        conn.close()
...
def _update_status(submission_id: int, status: str) -> bool:
    ...
    try:
        with conn:
            with conn.cursor() as cur:
                cur.execute(
                    "UPDATE newsletter_submissions SET status = %s WHERE id = %s",
                    (status, submission_id),
                )
        return True
    except Exception:
        logger.exception("[ADMIN] Failed to update newsletter submission status")
        return False
    finally:
        conn.close()
...
def _delete_submission(submission_id: int) -> bool:
    ...
    try:
        with conn:
            with conn.cursor() as cur:
                cur.execute("DELETE FROM newsletter_submissions WHERE id = %s", (submission_id,))
        return True
    except Exception:
        logger.exception("[ADMIN] Failed to delete newsletter submission")
        return False
    finally:
        conn.close()
...
def _render_campaign_list() -> str:
    ...
    if campaigns:
        ...
        for c in campaigns:
            ...
            sent = f"Sent {c['newsletter_date']}" if c["newsletter_date"] else "No newsletter date set"
            question = _esc(c["question"])
            question_html = (
                f'<div style="font-size:12px;color:#94a3b8;margin-top:4px;font-style:italic">“{question}”</div>'
                if question else ""
            )
            review_badge = (
            ...
            cards += f"""
            <a href="/admin?tab=newsletter&campaign_id={c['id']}"
              ...
              <div style="display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap">
                <div>
                  ...
                  <div style="font-size:12px;color:#64748b">{sent}</div>
                  {question_html}
                </div>
                <div style="display:flex;align-items:center;gap:14px">
                  {review_badge}
                  <span style="color:#94a3b8;font-size:13px">
                    <strong style="color:#67e8f9">{confirmed}</strong> confirmed
                  </span>
        ...
        <div style="text-align:center;padding:48px 24px;color:#4b5563">
          ...
          <div style="font-size:13px;color:#4b5563;max-width:460px;margin:0 auto;line-height:1.6">
            Create your first campaign below — one per newsletter issue that includes
            a "text the bot" CTA question.
          </div>
        </div>"""

    return f"""
    <div style="margin-bottom:8px;color:#94a3b8;font-size:13px;line-height:1.6">
      Each campaign is one newsletter issue's CTA question and the fan responses it pulled in.
      Click a campaign to review responses and pick the real ones.
    </div>
    ...
    <div style="background:#111827;border:1px solid #1f2937;border-radius:12px;padding:20px 24px">
      ...
      <form method="post" action="/admin/newsletter/campaign/new"
        ...
        <div style="display:flex;flex-direction:column;gap:5px;flex:2;min-width:200px">
          ...
          <input type="text" name="label" required maxlength="200" placeholder="e.g. July 2026 newsletter"
        ...
        <div style="display:flex;flex-direction:column;gap:5px;flex:1;min-width:140px">
          <label style="font-size:11px;color:#64748b">Newsletter Date (when it was sent)</label>
          <input type="date" name="newsletter_date"
                 style="background:#0f0f1a;border:1px solid #374151;border-radius:6px;color:#e2e8f0;
                        padding:9px 12px;font-size:14px;outline:none">
        </div>
        <div style="display:flex;flex-direction:column;gap:5px;flex:3;min-width:260px">
          <label style="font-size:11px;color:#64748b">CTA Question (what fans were told to text about)</label>
          <input type="text" name="question" required maxlength="500"
                 placeholder="e.g. What's the worst advice your mom ever gave you?"
                 style="background:#0f0f1a;border:1px solid #374151;border-radius:6px;color:#e2e8f0;
                        padding:9px 12px;font-size:14px;outline:none">
        </div>
        <button type="submit"
...
def _render_campaign_detail(campaign: dict) -> str:
    ...
    sent = f"Sent {campaign['newsletter_date']}" if campaign["newsletter_date"] else ""
    question = _esc(campaign["question"])
    question_html = (
        f'<div style="font-size:13px;color:#94a3b8;margin-top:4px;font-style:italic">CTA: “{question}”</div>'
        if question else ""
    )
    ...
    if to_review:
        ...
        review_body = _mini_empty(
            "Nothing to review.",
            f"Run the detection script to pull in responses:<br>"
            f"<code style='background:#0f0f1a;padding:3px 8px;border-radius:4px;font-size:12px'>"
            f"python scripts/detect_newsletter_responses.py --campaign-id {cid}</code>",
        )
    ...
    if confirmed:
        ...
        export_btn = (
            f'<a href="/admin/newsletter/export/{cid}" '
        ...
        confirmed_body = _mini_empty(
            ...
            "Check off the real responses on the left and they'll appear here.",
        )
        export_btn = ""
    ...
    <div style="margin-bottom:20px">
      <a href="/admin?tab=newsletter"
         style="color:#67e8f9;text-decoration:none;font-size:13px">← All campaigns</a>
      <div style="display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap;margin-top:8px">
        <div>
          ...
          <div style="font-size:12px;color:#64748b;margin-top:2px">{sent}</div>
          {question_html}
        </div>
        <div style="display:flex;gap:18px;align-items:center">
          ...
          <div style="text-align:center">
            <div style="font-size:20px;font-weight:700;color:#67e8f9">{len(confirmed)}</div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;align-items:start">
      ...
      <div style="background:#111827;border:1px solid #1f2937;border-radius:12px;padding:18px 20px">
        ...
        <div style="font-size:12px;color:#64748b;margin-bottom:14px">
          Tick the box on real CTA responses — they move to Confirmed →
        </div>
        {review_body}
      </div>
      ...
      <div style="background:#111827;border:1px solid #1f2937;border-radius:12px;padding:18px 20px">
        ...
        <div style="font-size:12px;color:#64748b;margin-bottom:14px">
          Your real responses for this issue. Mark them featured once used in a newsletter.
        </div>
        {confirmed_body}
      </div>
    ...
    <style>
      @media (max-width: 900px) {{
        #tab-newsletter [style*="grid-template-columns:1fr 1fr"] {{ grid-template-columns: 1fr !important; }}
      }}
    </style>
    """
...
def _review_row(sub: dict) -> str:
    fan_name = _esc(sub["contact_name"] or "")
    response = _esc(sub["response"])
    ...
    return f"""
    <tr id="nl-sub-row-{sub['id']}" style="border-bottom:1px solid #1a1a2e">
      <td style="padding:12px 8px 12px 0;vertical-align:top;width:28px">
        <input type="checkbox" onchange="nlConfirmSub({sub['id']})"
               style="width:18px;height:18px;cursor:pointer;accent-color:#0891b2"
               title="Mark as a real response">
      </td>
      <td style="padding:12px 0;vertical-align:top">
        ...
        <div style="color:#cbd5e1;line-height:1.5;font-size:13px">{response}</div>
        <div style="margin-top:6px;display:flex;gap:12px">
          <a href="/admin?tab=convos&thread={phone_q}" target="_blank"
             style="font-size:11px;color:#67e8f9;text-decoration:none">view thread</a>
          <button onclick="nlSkipSub({sub['id']})"
...
def _confirmed_row(sub: dict) -> str:
    ...
    dim = "opacity:.55;" if featured else ""
    featured_badge = (
        '<span style="background:#1a2e2e;color:#5eead4;border:1px solid #0f766e44;'
        'border-radius:99px;padding:2px 9px;font-size:10px;font-weight:700">FEATURED ✓</span>'
        if featured else ""
    )
    featured_toggle = (
        f'<button onclick="nlSetStatus({sub["id"]},\'confirmed\')" '
        f'style="font-size:11px;padding:0;border:none;background:none;color:#5eead4;cursor:pointer">undo featured</button>'
        if featured else
        f'<button onclick="nlSetStatus({sub["id"]},\'featured\')" '
        f'style="font-size:11px;padding:0;border:none;background:none;color:#67e8f9;cursor:pointer">mark featured ✓</button>'
    )
    return f"""
    <tr id="nl-sub-row-{sub['id']}" style="border-bottom:1px solid #1a1a2e;{dim}">
      <td style="padding:12px 0;vertical-align:top">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">
          {name_html}
        ...
        <div style="margin-top:6px;display:flex;gap:12px;flex-wrap:wrap">
          <a href="/admin?tab=convos&thread={phone_q}" target="_blank"
             style="font-size:11px;color:#67e8f9;text-decoration:none">view thread</a>
          {featured_toggle}
          <button onclick="nlSetStatus({sub['id']},'new')"
                  style="font-size:11px;padding:0;border:none;background:none;color:#6b7280;cursor:pointer">
            ← back to review
          </button>
        </div>
...
def _detail_js() -> str:
    ...
    function nlConfirmSub(id) {
      _nlPost('/admin/newsletter/submission/' + id + '/status', {status: 'confirmed'})
        .then(d => { if (d.ok) location.reload(); else alert('Failed'); });
    }
    function nlSkipSub(id) {
      _nlPost('/admin/newsletter/submission/' + id + '/status', {status: 'skip'})
        .then(d => { if (d.ok) { var el = document.getElementById('nl-sub-row-' + id); if (el) el.remove(); } else alert('Failed'); });
    }
    function nlSetStatus(id, status) {
      _nlPost('/admin/newsletter/submission/' + id + '/status', {status: status})
        .then(d => { if (d.ok) location.reload(); else alert('Failed'); });
    }
    </script>"""
...
def register_newsletter_routes(bp):
    from flask import jsonify

    @bp.route("/admin/newsletter/campaign/new", methods=["POST"])
    def newsletter_campaign_new():
        if not check_admin_auth():
            return require_admin_auth_response()
        label = request.form.get("label", "").strip()
        newsletter_date = request.form.get("newsletter_date", "").strip()
        question = request.form.get("question", "").strip()
        if label:
            _create_campaign(label, newsletter_date or None, question)
        return redirect("/admin?tab=newsletter")

    @bp.route("/admin/newsletter/submission/<int:sub_id>/status", methods=["POST"])
    ...
    @bp.route("/admin/newsletter/submission/<int:sub_id>/delete", methods=["POST"])
    def newsletter_submission_delete(sub_id):
        if not check_admin_auth():
            return require_admin_auth_response()
        ok = _delete_submission(sub_id)
        return jsonify({"ok": ok})

    @bp.route("/admin/newsletter/export/<int:campaign_id>")
    def newsletter_export(campaign_id):
        ...
        for r in rows:
            ...
            w.writerow([date_str, name, r["phone_number"][-4:], r["response"], r["status"]])
        return Response(
            buf.getvalue(),
            mimetype="text/csv",
